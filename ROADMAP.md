...
|-----|---------|------------------|---------------|
| **Gap 1: Runtime Sandboxing** | Obfuscated malware bypasses static analysis | v0.6.0: Docker hardening + behavior analysis | [YES] |
| **Gap 2: Safe-State Recovery** | Poisoned SHAs with no rollback points | v0.5.0: Pre-audit snapshots + recovery playbook | [YES] |
| **Gap 3: Dependency Conflicts** | Python/Node version conflicts crash agent | v0.7.0: Resolver + containerization | [YES] |
| **Gap 4: Web-of-Trust** | GitHub search: SEO poisoning + typo-squat | v0.8.0: Crypto verification + curated index | [YES] |
...
- **`reporter.py`** - Terminal rich table with [SAFE]/[REVIEW]/[BLOCKED] signals + per-result "why this for you" explanation. Saves `~/.agent-hunter/reports/hunt_report_YYYY-MM-DD.md`.
...
- `audit.py` - `agent-hunter audit`: reads installed skills from registry, for each:
 ...
 - Outputs health table: [SAFE] Healthy / [REVIEW] Update available / [BLOCKED] Security issue / ⚠️ Conflict
...
- On audit: if stored SHA ≠ current remote SHA → [BLOCKED] flag with explanation and rollback prompt
...
- Any hit → immediate [BLOCKED] exclusion, listed in report footer as "blocked by known-malicious index"
...
- GPL-3.0 skill + MIT project → [REVIEW] flag with explanation
- AGPL skill in any commercial context → [BLOCKED] flag
...
- Report findings as: `SANDBOX_ESCAPE_ATTEMPT` [BLOCKED] or `SUSPICIOUS_ENV_READ` 🟠
...
- New module: `scripts/dep_resolver.py`
 ...
 - If no compatible range exists, flags as `UNRESOLVABLE_CONFLICT` (severity: [BLOCKED])
...
- On audit: compare tested versions vs. host environment
 - Tested on Python 3.10, host is 3.12 → [REVIEW] compatibility warning (may work, not guaranteed)
 - Tested on Python 3.6, host is 3.12 → [BLOCKED] incompatible (likely broken)
...
- New module: `scripts/verify_sig.py`
 ...
 - Failed signature → [BLOCKED] flag "Signature mismatch - this skill may have been tampered"
