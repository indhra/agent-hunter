...
- ✅ Must work with NO local CLAUDE.md
- ✅ Global `~/.claude/CLAUDE.md` activation
- ✅ Project CLAUDE.md is optional, comes after first value
...
1. ❌ `update` command → merge into audit workflow
2. ❌ `scaffold` command → separate tool, not package manager
3. ❌ `resolve-deps` command → fold into install
4. ❌ Complex self-evolving language in docs
5. ❌ Over-detailed security marketing
6. ❌ Too many metadata fields in results
7. ❌ Dependency resolution complexity → v2
...
**Remove:**
- ❌ `scripts/scaffold.py`
- ❌ `scripts/dep_resolver.py` (v2)
- ❌ `scripts/mcp_parser.py` (merge into hunter)
- ❌ `scripts/typo_detect.py` (v2)
- ❌ `scripts/verify_sig.py` (v2)
- ❌ `scripts/update.py` (merge into audit)
- ❌ `scripts/release.py` (not user-facing)
...
Tasks:
1. ✅ Sync VERSION to 0.9.0 across all files
2. ✅ Rewrite README.md positioning (1-liner from PLAN.md)
3. ✅ Simplify SKILL.md (remove self-evolving language)
4. ✅ Remove placeholder media mentions
5. ✅ Align pyproject.toml metadata
6. ✅ Remove stale SPEC.md/ROADMAP.md sections
...
Tasks:
1. ✅ Remove scripts: scaffold.py, dep_resolver.py, typo_detect.py, verify_sig.py, update.py, release.py
2. ✅ Simplify scorer.py to 4 signals
3. ✅ Reduce security_scan.py to 10 core patterns
4. ✅ Path injection for testability
5. ✅ Top 3 default output in reporter.py
6. ✅ Improve "why this for you" explanations
...
Tasks:
1. ✅ Write unit tests for all scripts/ (pytest)
2. ✅ Integration tests for full hunt workflow
3. ✅ CLI tests for main.py commands
4. ✅ Edge case tests (errors, empty, failures)
5. ✅ Fixtures for mock data
6. ✅ Coverage report (pytest-cov)
...
Tasks:
1. ✅ Test on 10 real repos (FastAPI, React, ML, CLI, etc.)
2. ✅ Collect weak recommendation cases
3. ✅ Tune ranking for quality
4. ✅ Record demo video
5. ✅ Prepare 3 example runs
...
- **Hero:** "Hunt the right skills. Block the bad ones."
...
### Build it if
- ✅ Improves top 3 recommendation quality
- ✅ Improves trust
- ✅ Improves first-run clarity
- ✅ Improves install success
- ✅ Improves speed meaningfully

### Delay it if
- ❌ Adds complexity without user value
- ❌ Expands scope before quality is high
- ❌ Is mostly infrastructure vanity
- ❌ Is hard to explain in one sentence

### Cut it if
- ❌ Weakens product story
- ❌ Adds maintenance burden but no user pull
- ❌ Creates more claims than proof
...
1. ✅ Review this plan
2. ✅ Create new branch: `feat/plan-aligned-core`
3. ✅ Week 1: Truth restoration
4. ✅ Week 2: Core simplification
5. ✅ Week 3: Test coverage
6. ✅ Week 4: Real-world validation
7. ✅ Launch gate check
8. ✅ Ship v1.0
