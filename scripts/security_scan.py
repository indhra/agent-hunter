...
PROMPT_INJECTION_PATTERNS = [
    ...
    re.compile(r'do\s+not\s+(follow|obey|respect)\s+(the\s+)?(rules|guidelines|instructions)', re.I),
...
SHELL_EXEC_PATTERNS = [
    ...
    re.compile(r'subprocess\.(run|call|Popen|check_output)\s*\([\s\S]*?shell\s*=\s*True', re.MULTILINE),
...
ENV_EXFIL_PATTERNS = [
    ...
    re.compile(r'(?i)(os\.environ|os\.getenv)[\s\S]{0,200}?(requests\.|urllib|http\.client|socket\.connect|subprocess\.run|curl)'),
...
def scan_skill(
    ...
    # --- Full content checks ---
    _check_patterns(content, PROMPT_INJECTION_PATTERNS, "body", "SP-001", "RED",
                    "Prompt injection pattern in skill body", result)
    _check_patterns(content, SHELL_EXEC_PATTERNS, "body", "SP-002", "RED",
                    "Unguarded shell execution", result)
    _check_patterns(content, EVAL_EXEC_PATTERNS, "body", "SP-006", "YELLOW",
                    "Unguarded eval/exec", result)
    _check_patterns(content, SECRET_PATTERNS, "body", "SP-005", "RED",
                    "Embedded API key or secret token", result)
    _check_patterns(content, ENV_EXFIL_PATTERNS, "body", "SP-007", "YELLOW",
                    "Environment variable access", result)

    # --- Unicode checks ---
    if UNICODE_DIRECTION_OVERRIDE.search(content):
        result.findings.append(ScanFinding(
            pattern_id="SP-003", severity="RED",
            description="Hidden Unicode direction override character",
            location="body",
        ))
    if ZERO_WIDTH_CHARS.search(content):
        result.findings.append(ScanFinding(
            pattern_id="SP-004", severity="RED",
    ...
    if _has_homoglyphs(content):
        result.findings.append(ScanFinding(
            pattern_id="SP-008", severity="YELLOW",
